...
app.get('/:entity', authenticateToken, async (req: Request, res: Response) => {
  const entity: string = req.params.entity;
  ...
  try {
    ...
    return res.status(200).send({ status: 'success', data: result, message: `${result.length ?? 0} ${entity} get` });
  } catch (e: any) {
    return res.status(500).send({ status: 'error', message: e.message });
  }
});
...
// get single record
app.get('/:entity/:id', authenticateToken, async (req: Request, res: Response) => {
  const entity: string = req.params.entity;
  ...
  try {
    ...
    return res.status(200).send({ status: 'success', data: result, message: 'success' });
  } catch (e: any) {
    return res.status(500).send({ status: 'error', message: e.message });
  }
})
...
app.post('/:entity', authenticateToken, async (req: Request, res: Response) => {
  const entity: string = req.params.entity;
  const databaseTable: DatabaseTable = correspondingDatabaseTable(entity);
  if (!databaseTable) return res.status(400).send({ result: 'entity not found!' });
  // const result = await databaseTable.update(req.body);
  res.status(200).send({ result: 'ok' });
})

// delete single record
app.delete('/:entity', authenticateToken, async (req: Request, res: Response) => {
  const entity: string = req.params.entity;
  const databaseTable: DatabaseTable = correspondingDatabaseTable(entity);
  if (!databaseTable) return res.status(400).send({ result: 'entity not found!' });
